...
void *TSP_DFS(void* rank) {
  ...
  printf("rank thread = %ld of a Total of %d Threads With PThreads Static\n", my_rank, thread_count);
  pthread_mutex_unlock(&out);
  int interval = ((int)subtours.size()) / thread_count;
  // pthread_mutex_lock(&out);
  // db2(subtours.size(), interval);
  // pthread_mutex_unlock(&out);
  int start = my_rank * interval;
  int end = start + interval;

  // pthread_mutex_lock(&out);
  // printf("start = %d :: end = %d of thread Rank = %ld \n", start, end, my_rank);
  // pthread_mutex_unlock(&out);
